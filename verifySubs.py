...
if not status:
    ...
    numSubs = 3
    pipeline.setVariable('ArrayLength', numSubs)
    
    for iStream in range(numSubs):
        print >> sys.stderr, 'Creating Stream # %s of %s' % (iStream+1, numSubs)
        streamArgs = 'isComplete=%(completeness)s' % locals()
        pipeline.createSubStream("checkSubStream",iStream+1,streamArgs)
        continue
    pass
sys.exit(status)
